...
| `db/ingest-horsmonden.mjs` | Gate 4 — runs the REAL ingestion pipeline (Overpass + FHRS + Companies-House-if-key, via `src/lib/ingest/live.ts` bundled with esbuild) and lands results as `seed_proposals` only. Dedupes against accepted rows + pending proposals; idempotent. Set `CH_API_KEY` in `.env` to enable Companies House. | ongoing (per-community launch) | fold into `api/seed-ingest` admin trigger once that ships in the console |
